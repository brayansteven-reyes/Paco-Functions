...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    masterTableURL = "https://paco7input7data.blob.core.windows.net/input7data/Tabla_Maestra_v2.csv"
    logging.info('-------------- Geting data --------------')
    data = getDataFromURLAndReturnCSV(dataURL)
    masterTable = getDataFromURLAndReturnCSV(masterTableURL,";",'ISO-8859-1')
    logging.info('-------------- Process data --------------')
    result = processData(data,masterTable)
    logging.info('-------------- Database Conection --------------')
    cnx = connectToDatabase()
    logging.info('-------------- Upload data --------------')
    out = insertIntoDatabase(cnx,result,tableName)
...
def getDataFromURLAndReturnCSV(url,sep=",",encoding="UTF-8"):
...
def connectToDatabase():
    ...
    database_username = ''
    database_password = ''
    database_ip       = 'pacodb.mysql.database.azure.com'
    database_name     = 'paco'
    ssl_args = get_ssl_cert()
    cnx = sqlalchemy.create_engine('mysql+mysqlconnector://{0}:{1}@{2}/{3}'.
                                                format(database_username, database_password, 
                                                        database_ip, database_name), connect_args=ssl_args)
    logging.info(cnx)
    return cnx

def insertIntoDatabase(cnx,result,tableName):
    logging.info('-------------- insert --------------')
    resultQuery = cnx.execute('show databases')
    for row in resultQuery:
        logging.info(row)
    resultQuery = cnx.execute('show tables')
    for row in resultQuery:
        logging.info(row)
    result.to_sql(con=cnx, name=tableName, if_exists='append')
    return tableName

def get_ssl_cert():
    current_path = pathlib.Path(__file__).parent.parent
    return  {'ssl_ca': str(current_path / 'BaltimoreCyberTrustRoot.crt.pem')}